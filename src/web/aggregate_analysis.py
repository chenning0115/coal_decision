...
def aggregate_suggestion(gas_analysis_obj):
    ...
    for sug_id, sug_obj in id2suggests.items():
        item = "<div class='row alert alert-%s' role='alert'> \
                    ...
                    <div class='col-md-2'> \
                            <button id='suggest_click' onclick='suggest_func()' sug_id='%s' sug_title='%s' class='btn btn-%s' type='button'>%s</button> </div> \
                </div>" % (sug_obj.level, sug_obj.title, sug_obj.description, sug_id ,sug_obj.title, sug_obj.level, sug_obj.activate_title)
